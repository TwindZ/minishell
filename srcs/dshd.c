...
void	read_maker(t_data *data)
{
	...
	data->dshd.j++;
	while(var_char(data->dshd.templine[data->dshd.j]) && data->dshd.templine[data->dshd.j])
		data->dshd.j++;
	while(data->dshd.templine[data->dshd.j])
		data->readhd[data->dshd.i++] = data->dshd.templine[data->dshd.j++];
...
{	
	while(dollar(data->readhd))
	{
		...
		ft_strlcpy(data->dshd.var, data->dshd.templine + data->dshd.save_j + 1, data->dshd.varlen + 1);
		data->dshd.var = ft_strjoin(data->dshd.var, "=", 1);
		data->dshd.vardata = getenvp(data, data->dshd.var, 1);
		free(data->readhd);
		data->readhd = ft_calloc(ft_strlen(data->dshd.vardata) + ft_strlen(data->dshd.templine) + 2, sizeof(char)); 
		read_maker(data);
	}
}